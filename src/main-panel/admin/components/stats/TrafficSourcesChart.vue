...
// SVG icon paths for traffic sources
const SOURCE_ICONS: Record<string, { viewBox: string; path: string }> = {
  Instagram: {
    viewBox: '0 0 24 24',
    ...
    <div class="bar-chart">
      ...
      >
        <span class="bar-label">
          <svg v-if="getSourceIcon(src.source)" class="source-icon" :viewBox="getSourceIcon(src.source)!.viewBox">
            <path :d="getSourceIcon(src.source)!.path" :fill="COLORS[idx % COLORS.length]" />
          </svg>
          {{ src.source }}
        </span>
        <div class="bar-track">
